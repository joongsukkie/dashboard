...
2. **Cheap research priors** before they spend $5–25k on a real survey, conjoint panel, or A/B test. "Should we charge $69 or $89?" is currently answered with vibes; Lumen answers it with a synthetic study grounded in the company's own measured customer behaviour.
...
Generic synthetic-research tools (Synthetic Users, etc.) ask an LLM to role-play stereotypes. Lumen does the opposite. [`personas.py`](personas.py) builds **digital twins of the actual customer segments measured in the uploaded data** — each persona carries that segment's real AOV, price band, discount behaviour, repeat rate, category mix. The LLM is asked to reason **as that measured segment**, not as an invented persona.
